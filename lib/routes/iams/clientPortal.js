...
async function resolveClientProject(clientUser) {
  let projectId = clientUser.project_id;
  let clientId = clientUser.client_id;

  // Always check DB user to support instant credential / project updates without forcing re-login
  if (clientUser.user_id || clientUser.username) {
    try {
      ...
      }).select('assigned_project_id assigned_client_id');

      if (dbUser) {
        if (dbUser.assigned_project_id) projectId = dbUser.assigned_project_id;
        if (dbUser.assigned_client_id) clientId = dbUser.assigned_client_id;
      }
    } catch (e) {
  ...
  // 3. Fallback: normalize if ends with _main (e.g. prj_acrostone___wat_main -> prj_acrostone)
  if (!project && projectId && projectId.endsWith('_main')) {
    const baseId = projectId.replace(/_main$/, '').replace(/_+$/, '');
    project = await WorkspaceProject.findOne({
      $or: [
        { project_id: baseId },
        { project_id: new RegExp('^' + baseId.substring(0, 14), 'i') }
      ]
    });
  }
...
router.get('/messages', verifyClientAuth, async (req, res) => {
  try {
    ...
    if (!projectId) {
      return res.status(400).json({ success: false, error: 'No project associated with this account.' });
    }

    // Match messages by project_id OR channel_id to link with Admin Workspace Chat
    ...
    res.json({ success: true, count: messages.length, messages, data: messages });
  } catch (err) {
...
router.post('/messages', verifyClientAuth, async (req, res) => {
  try {
    ...
    const wsId = project?.workspace_id || 'ws_iceberg_master';
    ...
    const msg = new ChatMessage({
      ...
      sender: {
        user_id: req.clientUser.user_id,
        full_name: req.clientUser.full_name || client?.company_name || 'Client Representative',
        avatar_url: '',
        role: 'ClientGuest'
      },
      text: text.trim(),
    ...
    // Alert Account Manager / Agency Lead
    try {
      const notif = new Notification({
        recipient_role: 'ACCOUNT_MANAGER',
        title: `Message from ${client?.company_name || req.clientUser.full_name || 'Client'}`,
    ...
    res.status(201).json({ success: true, message: msg, data: msg });
  } catch (err) {
