...
/**
 * Automatically sync task file attachments to Docs and embedded links/comments to Bookmarks
 */
async function autoSyncTaskCrossLinks({ workspace_id, project_id, task, user }) {
  if (!task || !workspace_id || !project_id) return;
  const userName = user?.full_name || 'Team Member';
  const userId = user?.user_id || user?._id || 'usr_demo_admin';

  // 1. Sync file attachments into Docs
  if (Array.isArray(task.attachments)) {
    for (const att of task.attachments) {
      if (!att || !att.url) continue;
      const isLink = att.type === 'link' || (!att.is_image && !att.size?.includes('MB') && !att.size?.includes('KB') && att.url.startsWith('http') && !att.url.includes('data:'));
      if (!isLink) {
        try {
          const docTitle = att.name || 'Deliverable Asset';
          const existingDoc = await WorkspaceDoc.findOne({
            workspace_id,
            project_id,
          ...
          if (!existingDoc) {
            const isImg = !!att.is_image || /\.(png|jpe?g|gif|webp|svg)/i.test(docTitle) || (att.url && att.url.startsWith('data:image/'));
            await WorkspaceDoc.create({
              doc_id: 'doc_deliv_' + Math.random().toString(36).substring(2, 9),
              workspace_id,
              project_id,
              task_id: task.task_id,
              ...
              is_deliverable: true,
              file_url: att.url,
              file_size: att.size || 'Asset',
              file_type: att.type || (isImg ? 'image/png' : 'application/octet-stream'),
              content_html: `<div class="deliverable-doc-preview"><h3>${docTitle}</h3><p>Uploaded deliverable for task: <strong>${task.title}</strong></p></div>`,
              created_by: { user_id: userId, full_name: userName },
              last_edited_by: userName
            });
          }
        } catch (e) {
          console.warn('[AutoSync Doc Warn]:', e.message);
        }
      } else {
        // Sync link attachment into Bookmarks
        try {
          const existingBmk = await ProjectBookmark.findOne({
            workspace_id,
            project_id,
            url: att.url
          });
          if (!existingBmk) {
            await ProjectBookmark.create({
              bookmark_id: 'bmk_lnk_' + Math.random().toString(36).substring(2, 9),
              workspace_id,
              project_id,
              task_id: task.task_id,
              task_title: task.title,
              ...
              description: `Embedded link in task "${task.title}"`,
              notes: `Task deliverable link for ${task.title}`,
              tags: ['Task Link', 'Deliverable'],
              created_by: { user_id: userId, full_name: userName }
            });
          }
        } catch (e) {
          console.warn('[AutoSync Bookmark Warn]:', e.message);
        }
      }
    }
  }

  // 2. Extract links from task comments and sync to Bookmarks
  if (Array.isArray(task.comments)) {
    const urlRegex = /https?:\/\/[^\s<"']+/gi;
    for (const cmt of task.comments) {
      if (!cmt || !cmt.text) continue;
      const matches = cmt.text.match(urlRegex) || [];
        ...
        try {
          ...
          if (!existingBmk) {
            await ProjectBookmark.create({
              ...
              url,
              title: url,
              description: `Shared by ${cmt.author_name || userName} in task comment`,
              notes: `Discussion link from ${task.title}`,
              tags: ['Comment Link', 'Task Resource'],
              created_by: { user_id: cmt.author_id || userId, full_name: cmt.author_name || userName }
            });
          }
        } catch (e) {
          console.warn('[AutoSync Comment Link Warn]:', e.message);
        }
      }
    }
  }
...
router.post('/', async (req, res) => {
  try {
    ...
    await logAudit({
      ...
      entityId: newWorkspace.workspace_id,
      severity: 'INFO',
      after: newWorkspace.toObject(),
      details: { name: newWorkspace.name, slug: newWorkspace.slug }
    });
    await logUserEvent({
      req,
...
router.post('/:workspace_id/members', requireWorkspaceRole('ADMIN'), async (req, res) => {
  try {
    ...
    await logUserEvent({
      req,
      eventType: 'ADD_WORKSPACE_MEMBER',
      entityType: 'WORKSPACE',
      entityId: workspace.workspace_id
    });
...
router.post('/:workspace_id/projects', requireWorkspaceRole('ADMIN', 'MEMBER'), async (req, res) => {
  try {
    const wsId = req.workspace.workspace_id;
    const userId = req.user.user_id || req.user._id || 'usr_demo_admin';
    const { name, description, color, icon, client_id, category, cms_project_id, enabled_tools, sections } = req.body;
    ...
    const newProject = new WorkspaceProject({
      ...
      cms_project_id: cms_project_id || null,
    ...
    await logAudit({
      ...
      details: { name: newProject.name, workspace_id: wsId }
    });
    await logUserEvent({
      req,
      eventType: 'CREATE_WORKSPACE_PROJECT',
      entityType: 'WORKSPACE_PROJECT',
      entityId: newProject.project_id
    });
...
router.put('/:workspace_id/projects/:project_id', scopedProjectAccess('project_id'), async (req, res) => {
  try {
    ...
    const existing = await WorkspaceProject.findOne({ project_id, workspace_id: req.workspace.workspace_id });
    if (!existing) {
    ...
    await logUserEvent({
      req,
      eventType: 'EDIT_WORKSPACE_PROJECT',
      entityType: 'WORKSPACE_PROJECT',
      entityId: project.project_id
    });
...
router.post('/:workspace_id/tasks', async (req, res) => {
  try {
    ...
    // Auto-sync cross-links (attachments -> Docs, links -> Bookmarks)
    autoSyncTaskCrossLinks({
      workspace_id: wsId,
      project_id,
      task: newTask,
      user: req.user
    }).catch(e => console.warn('Cross-link auto-sync warn:', e.message));

    await logAudit({
      req,
      action: 'TASK_CREATED',
      entityType: 'WORKSPACE_TASK',
      entityId: newTask.task_id,
      severity: 'INFO',
      after: newTask.toObject(),
      details: { title: newTask.title, project_id, status }
    });
    await logUserEvent({
      req,
      eventType: 'CREATE_WORKSPACE_TASK',
      entityType: 'WORKSPACE_TASK',
      entityId: newTask.task_id
    });
...
router.put('/:workspace_id/tasks/:task_id/reorder', async (req, res) => {
  try {
    ...
    await task.save();
    const afterState = task.toObject();
    ...
    await logUserEvent({
      req,
      eventType: 'REORDER_WORKSPACE_TASK',
      entityType: 'WORKSPACE_TASK',
      entityId: task.task_id
    });
...
router.put('/:workspace_id/tasks/:task_id', async (req, res) => {
  try {
    ...
    const existing = await WorkspaceTask.findOne({ task_id, workspace_id: req.workspace.workspace_id });
    if (!existing) {
      return res.status(404).json({ success: false, error: 'Task not found.' });
    }
...
router.delete('/:workspace_id/tasks/:task_id', async (req, res) => {
  try {
    ...
    await logAudit({
      ...
      entityId: task_id,
      severity: 'WARN',
      before: task ? task.toObject() : null,
      details: { task_id, title: task?.title }
    });
    await logUserEvent({
      req,
      eventType: 'DELETE_WORKSPACE_TASK',
      entityType: 'WORKSPACE_TASK',
      entityId: task_id
    });
...
/**
 * GET /api/iams/workspaces/:workspace_id/tasks/:task_id/attachments/:attachment_id/download
 * Download attachment directly from task attachments
 */
router.get('/:workspace_id/tasks/:task_id/attachments/:attachment_id/download', async (req, res) => {
  try {
    ...
    const att = (task.attachments || []).find(a => a.attachment_id === attachment_id || a.name === req.query.name);
    if (!att) {
    ...
    if (att.url && att.url.startsWith('data:')) {
      ...
      if (parts.length === 2) {
        ...
        res.setHeader('Content-Type', mime || att.mime_type || 'application/octet-stream');
        res.setHeader('Content-Length', buffer.length);
        return res.end(buffer);
      }
    }

    const fallback = `ICEBERG DELIVERABLE: ${filename}\nTask: ${task.title}\nTimestamp: ${new Date().toISOString()}`;
    const buffer = Buffer.from(fallback, 'utf-8');
