...
// Middleware to ensure database connection is ready for all operations
router.use(async (req, res, next) => {
  if (mongoose.connection.readyState !== 1) {
    try {
      const rawUri = (process.env.MONGODB_URI || '').trim().replace(/[\r\n]/g, '');
      const uri = rawUri || 'mongodb+srv://hmanmahmed_db_user:V4lKIpvmjTI7nn3K@iceberg.4dboitw.mongodb.net/iceberg_cms?retryWrites=true&w=majority';
      await mongoose.connect(uri, { serverSelectionTimeoutMS: 5000, connectTimeoutMS: 5000 });
    } catch (dbErr) {
      console.warn('[IAMS Employees DB Connect Warning]:', dbErr.message);
    }
  }
  next();
});
...
router.get('/', executiveOnly, async (req, res) => {
  try {
    ...
    // Strictly query internal agency workforce only — exclude client users & representatives
    const filter = {
      role: { $nin: ['ClientGuest', 'CLIENT_VIEWER', 'GUEST'] },
      $and: [
        {
          $or: [
    ...
    if (role && !['ClientGuest', 'CLIENT_VIEWER', 'GUEST'].includes(role)) filter.role = role;
...
router.post('/', executiveOnly, async (req, res) => {
  try {
    ...
    if (['ClientGuest', 'CLIENT_VIEWER', 'GUEST'].includes(role)) {
      return res.status(400).json({ success: false, error: 'Client representative accounts cannot be created in the agency manpower directory. Manage clients in the Clients section.' });
    }
...
/**
 * DELETE /api/iams/employees/:id
 * Permanently delete an employee account from the database (CEO Only)
 */
router.delete('/:id', executiveOnly, async (req, res) => {
  try {
    const { id } = req.params;

    const employee = await User.findOne({
      $or: [{ _id: mongoose.Types.ObjectId.isValid(id) ? id : null }, { user_id: id }]
    });
    ...
    // Security check: Prevent deleting self
    const reqUserId = req.user?.user_id || req.user?.id || req.user?._id;
    if (reqUserId && (employee.user_id === reqUserId || employee._id.toString() === reqUserId.toString())) {
      return res.status(400).json({ success: false, error: 'Security constraint: You cannot delete your own executive account.' });
    }

    // Security check: Prevent deleting root system accounts
    const protectedEmails = ['admin@icebergma.com', 'admin@iceberg.agency'];
    if (protectedEmails.includes(employee.email?.toLowerCase())) {
      return res.status(400).json({ success: false, error: 'Security constraint: Root administrator account cannot be deleted.' });
    }

    const deletedName = employee.full_name;
    const deletedEmail = employee.email;
    const deletedUserId = employee.user_id;

    // Permanently remove from User collection
    await User.deleteOne({ _id: employee._id });

    // Unassign tasks previously allocated to this employee
    try {
      await Task.updateMany(
        { assigned_to_id: employee._id },
        { $set: { assigned_to_id: null, assigned_to_name: 'Unassigned' } }
      );
      await WorkspaceTask.updateMany(
        { 'assignees.user_id': employee.user_id },
        { $pull: { assignees: { user_id: employee.user_id } } }
      ).catch(() => {});
    } catch (unassignErr) {
      console.warn('[IAMS Employee Delete Task Unlink Warn]:', unassignErr.message);
    }
    ...
    await logAudit({
      ...
      severity: 'HIGH',
      details: { full_name: deletedName, email: deletedEmail, deleted_by: req.user?.email || 'CEO' }
    });
    await logUserEvent({
      req,
    ...
    res.json({
      success: true,
      message: `Employee account "${deletedName}" (${deletedEmail}) has been permanently deleted from the database.`
    });
  } catch (err) {
    console.error('[IAMS Employee Delete Error]:', err);
    res.status(500).json({ success: false, error: err.message || 'Failed to delete employee account.' });
  }
});
...
router.get('/performance', executiveOnly, async (req, res) => {
  try {
    ...
    }).select('user_id full_name email role department avatar_url cost_rates capacity performance_reviews okrs');
    const allTasks = await Task.find({}).lean();
    const allWorkspaceTasks = await WorkspaceTask.find({}).lean().catch(() => []);
    const allDocs = await WorkspaceDoc.find({}).lean().catch(() => []);
    const allBookmarks = await ProjectBookmark.find({}).lean().catch(() => []);
    const allTopics = await MessageTopic.find({}).lean().catch(() => []);
    ...
    const performanceReport = employees.map(emp => {
      ...
      const wsAssigned = allWorkspaceTasks.filter(wt => {
        ...
        return wt.assignees.some(a => a.user_id === empUserId || (a.full_name && a.full_name.toLowerCase() === empName));
      });

      const totalTasks = assignedTasks.length + wsAssigned.length;
      const completedTasks = assignedTasks.filter(t => t.status === 'DONE').length + wsAssigned.filter(t => t.status === 'DONE' || t.status === 'COMPLETED' || t.status === 'approved').length;
      ...
      wsAssigned.forEach(wt => {
        if (Array.isArray(wt.subtasks)) {
          totalSubtasks += wt.subtasks.length;
          completedSubtasks += wt.subtasks.filter(st => st.completed).length;
        }
      });
      const subtaskRate = totalSubtasks > 0 ? Math.round((completedSubtasks / totalSubtasks) * 100) : 100;
      ...
      wsAssigned.forEach(wt => {
        estimatedHours += (Number(wt.duration_minutes) || 0) / 60;
        actualHours += (Number(wt.logged_minutes) || 0) / 60;
      });
      estimatedHours = Number(estimatedHours.toFixed(1));
      actualHours = Number(actualHours.toFixed(1));

      // On-time completion estimate
      const overdueTasks = [...assignedTasks, ...wsAssigned].filter(t => t.due_date && new Date(t.due_date) < new Date() && t.status !== 'DONE' && t.status !== 'COMPLETED').length;
      const onTimeRate = totalTasks > 0 ? Math.max(0, Math.round(((totalTasks - overdueTasks) / totalTasks) * 100)) : 100;
      const completionRate = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 100;

      // Deliverables & Docs contributed
      const userDocs = allDocs.filter(d => {
        const creatorId = d.created_by?.user_id || (typeof d.created_by === 'string' ? d.created_by : '');
        const editor = (d.last_edited_by || '').toLowerCase();
        return creatorId === empUserId || editor === empName || (d.task_id && wsAssigned.some(wt => wt.task_id === d.task_id));
      });

      // Resource Bookmarks shared
      const userBookmarks = allBookmarks.filter(b => {
        const creatorId = b.created_by?.user_id || (typeof b.created_by === 'string' ? b.created_by : '');
        return creatorId === empUserId || (b.task_id && wsAssigned.some(wt => wt.task_id === b.task_id));
      });

      // Async discussions and topic replies
      let userTopicsCount = 0;
      let userRepliesCount = 0;
      allTopics.forEach(top => {
        if (top.author?.user_id === empUserId || (top.author?.full_name || '').toLowerCase() === empName) {
          userTopicsCount++;
        }
        if (Array.isArray(top.replies)) {
          userRepliesCount += top.replies.filter(r => (r.author?.user_id === empUserId || (r.author_name || '').toLowerCase() === empName)).length;
        }
      });
      const totalDiscussions = userTopicsCount + userRepliesCount;

      // Composite Collaboration & Execution Score (0-100)
      // Weighted: 40% task completion, 20% on-time delivery, 20% subtask execution, 20% collaboration (docs, bookmarks, discussions)
      const collabActivityScore = Math.min(100, (userDocs.length * 10) + (userBookmarks.length * 8) + (totalDiscussions * 6));
      const compositeScore = Math.round(
        (completionRate * 0.40) +
        (onTimeRate * 0.20) +
        (subtaskRate * 0.20) +
        (collabActivityScore * 0.20)
      );
      ...
      return {
        ...
        metrics: {
          ...
          docs_produced_count: userDocs.length,
          bookmarks_shared_count: userBookmarks.length,
          discussions_count: totalDiscussions,
          collaboration_score: Math.min(100, Math.max(0, compositeScore)),
          estimated_hours: estimatedHours,
          actual_hours: actualHours,
          hours_variance: Number((actualHours - estimatedHours).toFixed(1)),
