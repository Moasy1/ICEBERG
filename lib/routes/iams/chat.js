...
router.get('/messages', async (req, res) => {
  try {
    ...
    if (project_id) {
      // Direct link with Client Portal: match either project_id or channel_id
      query.$or = [
...
router.post('/messages', async (req, res) => {
  try {
    const wsId = req.workspace.workspace_id || 'ws_iceberg_master';
    const userId = req.user.user_id || req.user._id || 'usr_demo_admin';
    ...
    const newChatMessage = new ChatMessage({
      ...
      channel_id: channel_id || project_id || (recipient_id ? null : 'general'),
      recipient_id: recipient_id || null,
      sender: {
        user_id: userId,
        full_name: req.user.full_name || 'Team Member',
        avatar_url: req.user.avatar || req.user.avatar_url || '',
