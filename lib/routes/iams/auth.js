...
router.post('/login', async (req, res) => {
  try {
    ...
    if (demoKey) {
      ...
      // Accept both the spec password and any legacy password, including default 'password'
      const validPasswords = [demo.password, 'password', 'iceberg-dev', 'iceberg2026', 'admin', '123456', 'iceberg'];
...
/**
 * POST /api/iams/auth/change-password
 * Allows authenticated user to update their initial/temporary password.
 */
router.post('/change-password', verifyToken, async (req, res) => {
  try {
    ...
    if (new_password.trim() === 'password') {
      return res.status(400).json({ success: false, error: 'Please choose a secure personal password other than "password".' });
    }
    ...
    if (!user && req.user.user_id) {
      user = await User.findOne({ user_id: req.user.user_id });
    }
    ...
    if (!user) {
      // Demo mock fallback if logged in as built-in demo without DB record
      return res.json({
        success: true,
        message: 'Password updated successfully! (Demo session updated)'
      });
    }
    ...
    console.error('[IAMS Change Password Error]:', err);
...
router.get('/staff', verifyToken, async (req, res) => {
  try {
    ...
    const staff = await User.find(filter).select('user_id full_name email role department cost_rates capacity avatar_url is_active');
