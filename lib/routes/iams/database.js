...
let lastCollectionsCacheTime = 0;
let isRefreshingCollections = false;
const CACHE_TTL_MS = 60 * 1000; // 60s fresh
const CACHE_STALE_MS = 15 * 60 * 1000; // 15m allowed stale

async function fetchCollectionsList() {
  const entries = Object.entries(mongoose.models);
  const collectionsList = await Promise.all(entries.map(async ([modelName, model]) => {
    try {
      const collectionName = model.collection?.collectionName || modelName;
      // Race each count with a 1500ms timeout so slow network never hangs the entire list
      const docCount = await Promise.race([
        model.estimatedDocumentCount().catch(() => model.countDocuments().catch(() => 0)),
        new Promise(resolve => setTimeout(() => resolve(0), 1500))
      ]);
      const indexCount = model.schema?.indexes ? model.schema.indexes().length + 1 : 1;
  ...
  cachedCollections = collectionsList;
  lastCollectionsCacheTime = Date.now();
  return collectionsList;
}

function triggerBackgroundRefresh() {
  if (isRefreshingCollections) return;
  isRefreshingCollections = true;
  fetchCollectionsList()
    .catch(err => console.warn('[IAMS DB Cache Refresh Warn]:', err.message))
    .finally(() => { isRefreshingCollections = false; });
}

/**
 * Warm up collections cache on server boot
 */
 ...
 * Fast health check for DB status, total documents, and volume breakdown
 */
router.get('/health', async (req, res) => {
  try {
    ...
    if (!isFresh && cachedHealth && (now - lastHealthCacheTime < 30000)) {
      return res.json({
        success: true,
        health: cachedHealth,
        cached: true
    ...
    if (mongoose.connection.readyState !== 1) {
      let rawUri = (process.env.MONGODB_URI || '').trim().replace(/[\r\n]/g, '');
      if (rawUri.includes('V41kIpvmjTI7nn3K')) rawUri = rawUri.replace('V41kIpvmjTI7nn3K', 'V4lKIpvmjTI7nn3K');
      ...
      await mongoose.connect(uri, { serverSelectionTimeoutMS: 4000, connectTimeoutMS: 4000 }).catch(() => {});
    }

    const connectionState = mongoose.connection.readyState === 1
      ? 'CONNECTED'
      : (mongoose.connection.readyState === 2 ? 'CONNECTING' : 'DISCONNECTED');

    let colls = cachedCollections;
    if (!colls) {
      colls = await fetchCollectionsList().catch(() => []);
    }

    const totalCollections = colls ? colls.length : Object.keys(mongoose.models).length;
    const totalDocuments = colls ? colls.reduce((sum, c) => sum + (c.document_count || 0), 0) : 0;
    ...
    try {
      ...
      if (AuditLog && mongoose.connection.readyState === 1) {
        ...
        writesLastHour = await Promise.race([
          AuditLog.countDocuments({ created_at: { $gte: oneHourAgo } }).catch(() => 0),
...
router.get('/collections', async (req, res) => {
  try {
    ...
    // 1. Fresh cache hit (< 60s) -> return in 0ms
    if (!isFresh && cachedCollections && (now - lastCollectionsCacheTime < CACHE_TTL_MS)) {
      return res.json({
        success: true,
        count: cachedCollections.length,
        collections: cachedCollections,
        cached: true
      });
    }

    // 2. Stale-While-Revalidate hit (< 15m) -> return stale cache in 0ms, refresh asynchronously
    if (!isFresh && cachedCollections && (now - lastCollectionsCacheTime < CACHE_STALE_MS)) {
      triggerBackgroundRefresh();
      return res.json({
        success: true,
        count: cachedCollections.length,
    ...
    // 3. Cold start or fresh=true -> fetch live
    const collectionsList = await fetchCollectionsList();
...
router.get('/collections/:name/documents', async (req, res) => {
  try {
    ...
    const [total, documents] = await Promise.all([
      q ? model.countDocuments(query) : model.estimatedDocumentCount().catch(() => model.countDocuments({})),
...
router.get('/health', async (req, res) => {
  try {
    const stats = {};
    const entries = Object.entries(mongoose.models);
    const collectionBreakdown = await Promise.all(entries.map(async ([modelName, model]) => {
      try {
        const count = await model.estimatedDocumentCount().catch(() => model.countDocuments().catch(() => 0));
        return {
          collection: model.collection.collectionName,
          model: modelName,
          count
        ...
        return {
          collection: model.collection?.collectionName || modelName,
          model: modelName,
          count: 0
        };
      }
    ...
    let totalDocs = 0;
    for (const item of collectionBreakdown) {
