...
router.get('/board', verifyToken, async (req, res) => {
  try {
    ...
    if (assigned_to) {
      const assignedArr = assigned_to.split(',').map(s => s.trim()).filter(Boolean);
      const validObjectIds = assignedArr.filter(id => mongoose.Types.ObjectId.isValid(id));
      const userLookups = await User.find({
        $or: [
          { _id: { $in: validObjectIds } },
          { user_id: { $in: assignedArr } },
          { email: { $in: assignedArr } }
        ]
      }).select('_id');
      const resolvedIds = [...new Set([...validObjectIds, ...userLookups.map(u => u._id.toString())])];
      if (resolvedIds.length === 1) {
        filter.assigned_to_id = resolvedIds[0];
      } else if (resolvedIds.length > 1) {
        filter.assigned_to_id = { $in: resolvedIds };
      }
    }
...
router.patch('/:id/status', verifyToken, async (req, res) => {
  try {
    ...
    await logAudit({
      ...
      before: beforeState,
      after: afterState,
      details: { oldStatus, newStatus: status, title: task.title }
    });
    await logUserEvent({
      req,
      eventType: 'EDIT_TASK',
...
router.post('/:id/upload-version', verifyToken, async (req, res) => {
  try {
    ...
    await logAudit({
      req,
      action: 'TASK_VERSION_UPLOADED',
      entityType: 'TASK',
      entityId: task.task_id,
      severity: 'INFO',
      details: { version_number: versionNumber, asset_url, preview_type }
    });
    await logUserEvent({
      req,
      eventType: 'UPLOAD_TASK_VERSION',
      entityType: 'TASK',
      entityId: task.task_id
    });
...
router.post('/:id/client-review', async (req, res) => {
  try {
    ...
    await logAudit({
      ...
      entityId: task.task_id,
      severity: 'INFO',
      details: { client_status, client_feedback }
    });
    await logUserEvent({
      req,
      eventType: 'SUBMIT_TASK_REVIEW',
      entityType: 'TASK',
      entityId: task.task_id
    });
...
router.post('/:id/log-hours', verifyToken, async (req, res) => {
  try {
    ...
    await logAudit({
      ...
      entityId: task.task_id,
      severity: 'INFO',
      details: { hours_logged: hoursNum, total_actual_hours: task.time_tracking.actual_hours }
    });
    await logUserEvent({
      req,
      eventType: 'LOG_HOURS',
