...
const { logAuditAsync, logUserEventAsync } = require('../utils/audit');

// Ensure upload directory exists safely (use /tmp on Vercel/serverless)
...
router.post('/submit', handleUpload, async (req, res) => {
  try {
    ...
    logAuditAsync({
      ...
      entityId: email,
      severity: 'INFO',
      details: { name, email, company: finalBusinessName, appointmentDate, appointmentTime }
    });
    logUserEventAsync({
      req,
      eventType: 'SUBMIT_CONTACT',
      entityType: 'CONTACT',
      entityId: email
    });
